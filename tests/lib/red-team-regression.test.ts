...
beforeEach(() => {
  ...
  // Clear both Claude Code env signals so tests that exercise the filesystem
  // walk aren't short-circuited by the ambient CC env (e.g., when tests run
  // inside Claude Code's bash sandbox where CLAUDECODE=1 is always set).
