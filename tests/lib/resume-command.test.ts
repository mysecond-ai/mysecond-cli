...
describe('buildResumeCommand', () => {
  ...
  it('echoes a direct node invocation when running from the fallback install dir', () => {
    process.argv[1] = '/Users/pm/.mysecond/cli/1.11.0/bin/mysecond.cjs';
    const cmd = buildResumeCommand('acme-x1');
    expect(cmd).toBe(
      'MYSECOND_CUSTOMER_SLUG=acme-x1 node /Users/pm/.mysecond/cli/1.11.0/bin/mysecond.cjs init --resume'
    );
  });
